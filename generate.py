  ...
  <style>
    ...
    *,*::before,*::after{margin:0;padding:0;box-sizing:border-box}
    ...
    .cmd-avatar{
      ...
      background:radial-gradient(circle at 50% 35%,#14142a 0%,#04040e 80%);
      border:1.5px solid rgba(56,189,248,.3);
      box-shadow:0 0 22px rgba(56,189,248,.15),0 0 60px rgba(56,189,248,.05),inset 0 0 16px rgba(0,0,0,.8);
      position:relative;overflow:hidden}
    .cmd-avatar::before{
    ...
    .cmd-avatar::after{
      content:'';position:absolute;
      width:52px;height:32px;border-radius:50% 50% 0 0 / 60% 60% 0 0;
      background:rgba(5,5,16,.9);
      bottom:-2px;left:50%;transform:translateX(-50%)}
    /* Title block */
    .cmd-title{flex:1}
    .cmd-label{font-size:10px;font-weight:700;letter-spacing:.15em;
      text-transform:uppercase;color:var(--blue);margin-bottom:5px;
      display:flex;align-items:center;gap:8px}
    .cmd-label::before{content:'◈';font-size:8px;animation:glow 2s ease infinite}
    .cmd-name{font-size:22px;font-weight:900;color:#fff;letter-spacing:-.02em;line-height:1.1}
    .cmd-sub{font-size:12px;color:var(--td);margin-top:5px}
    .cmd-status{display:flex;align-items:center;gap:6px;margin-top:8px;font-size:11px;color:var(--td)}
    .status-pulse{width:7px;height:7px;border-radius:50%;background:var(--green);animation:pulse 2s infinite}
    /* Meta */
    .cmd-meta{text-align:right;flex-shrink:0}
    .cmd-updated{font-size:12px;color:var(--td)}
    .cmd-auto{display:inline-flex;align-items:center;gap:4px;font-size:10px;
      background:rgba(139,111,255,.12);color:var(--pl);
      padding:2px 8px;border-radius:8px;margin-top:4px}
    .cmd-edit{display:block;font-size:11px;color:var(--purple);margin-top:5px;text-decoration:none}
    .cmd-edit:hover{text-decoration:underline}
    ...
    .kpi-bg-icon{position:absolute;right:14px;top:50%;transform:translateY(-50%);
      font-size:48px;opacity:.04;pointer-events:none;user-select:none}
    .kpi-card.ai-glow{background:linear-gradient(135deg,var(--s1) 0%,#140e28 100%);
      border-color:rgba(139,111,255,.3)}
    .kpi-card.ai-glow::before{content:'';position:absolute;inset:0;
      background:linear-gradient(90deg,transparent,rgba(139,111,255,.06),transparent);
      background-size:200% 100%;animation:shimmer 3s linear infinite}
    .kpi-label{font-size:11px;color:var(--td);margin-bottom:8px;font-weight:500}
    .kpi-desc{font-size:9px;color:var(--tm);margin-bottom:8px}
    .kpi-value{font-size:30px;font-weight:900;color:#fff;line-height:1;position:relative}
    .kpi-sub{font-size:11px;color:var(--td);margin-top:8px}
    ...
    /* ── Responsive ── */
    @media(max-width:1200px){.kpi-grid,.fin-grid{grid-template-columns:repeat(2,1fr)}}
    @media(max-width:900px){body{padding:16px}.project-grid{grid-template-columns:repeat(2,1fr)}.auto-grid{grid-template-columns:1fr}.two-col{grid-template-columns:1fr}}
    @media(max-width:600px){.kpi-grid{grid-template-columns:1fr 1fr}.project-grid{grid-template-columns:1fr}.fin-grid{grid-template-columns:1fr 1fr}}
    ...
    [data-theme="strategist"] .kpi-card.ai-glow{background:linear-gradient(135deg,#fff 0%,#f0eef8 100%);border-color:rgba(90,58,138,.2)}
    [data-theme="strategist"] .kpi-card.ai-glow::before{background:linear-gradient(90deg,transparent,rgba(90,58,138,.06),transparent)}
    ...
    .th-btn{width:30px;height:30px;border-radius:8px;border:1px solid var(--border);background:var(--s2);
      color:var(--td);font-size:14px;cursor:pointer;display:flex;align-items:center;justify-content:center;
...
<!-- ══ 司令塔 ヘッダー ══════════════════════════════════════ -->
<div class="cmd-header fade-in">
  <div class="cmd-avatar"></div>
  <div class="cmd-title">
    <div class="cmd-label">AI カンパニー 統合司令室</div>
    <div class="cmd-name">岩嵜浩之</div>
    <div class="cmd-status">
      <span class="status-pulse"></span>
      全システム稼働中 — <span id="month-label" style="margin-left:4px"></span>
    </div>
  </div>
  <div style="flex:1"></div>
  <div class="cmd-meta">
    <div class="cmd-updated">最終更新: [[LAST_UPDATED]]</div>
    <div class="cmd-auto">⟳ 毎朝 8:00 JST 自動更新</div>
    <a class="cmd-edit" href="data.json" target="_blank">✏️ data.json を編集</a>
    <div style="font-size:9px;color:var(--tm);text-align:right;margin-top:8px">スタイル切替</div>
...
<div class="section fade-in d1">
  ...
  <div class="kpi-grid">
    ...
    <div class="card kpi-card">
      ...
      <div class="kpi-desc">Monthly Recurring Revenue — 毎月安定して入ってくる売上</div>
...
<script>
  ...
  const greets = [[5,12,'おはようございます'],[12,18,'こんにちは'],[18,22,'お疲れ様です'],[0,24,'遅くまでお疲れ様です']];
